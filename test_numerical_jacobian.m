...
function test_numerical_jacobian(jfun)
    ...
    for n = 1:num_tests
        ...
        %if J is close to A, test has succeeded
    ...
    %display results
    disp(' ');
    disp('Numerical Jacobian Test:')
    disp(['Results: ',num2str(num_success),' out of ',...
          num2str(num_tests),' tests passed.']);
    disp(' ');
end
